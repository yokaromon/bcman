...
`/bcman/mobile-scan-lab/`を開くと、動画から1フレームを選び、このAPIの結果を元画像へ
オーバーレイする非保存テストができます。動画は連続送信せず、「この状態を高解像度で抽出」を
押した瞬間だけ送信します。テスト画面は`semantic=true`を指定し、OpenCV候補を個別に
台形補正した候補一覧シートを管理下Visionモデルで1回だけ評価します。重なった候補線の混同を
避けつつ、1枚の名刺全体を囲む候補だけを残した後、採用候補の四隅を原寸画像の局所領域で
再調整します。候補画像や応答も保存しません。

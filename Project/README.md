...
ログイン後の撮影画面にある「動画から矩形検出を試す」、または
`/bcman/mobile-scan-lab/`を開くと、中央ガイドへ名刺を1枚ずつ合わせて取得する非保存テストが
できます。JavaScriptはガイド範囲の鮮明度とフレーム間の静止を確認し、条件を満たした時だけ
その範囲をJPEG化して`POST /bcman/api/guided-card-captures`へ送ります。動画の連続送信は
行いません。

サーバはガイド全体を保険候補にしつつ、範囲内の局所輪郭を検出します。候補を個別に台形補正した
一覧シートを管理下Visionモデルで1回評価し、名刺1枚と確認できた精密候補を優先して原寸四隅を
再調整します。応答には補正済みJPEGのdata URL、品質値、重複判定用fingerprintを含みます。
ブラウザは取得済み名刺をメモリ上の一覧へ並べ、同じ名刺の連続取得を抑止します。原画像、候補、
切り抜き、応答、Photo/Card行はいずれもサーバへ保存しません。

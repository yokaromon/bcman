...
from .models import BusinessCard, Contact, Group, OCRResult, Organization, Photo, User
from .schemas import ContactInput, GroupInput, OrganizationInput, ReprocessInput, UserInput
from .services import detect_cards, image_size, process_photo, run_ocr, structure
...
@app.get("/api/photos")
def list_photos(db: Session=Depends(get_db), user: User=Depends(current_user)): return [{"id":p.id,"filename":p.original_filename,"status":p.status,"created_at":p.created_at} for p in visible_photo_query(db,user).order_by(Photo.created_at.desc())]

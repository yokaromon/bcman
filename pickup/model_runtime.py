...
DEFAULT_CACHE_DIR = BASE_DIR / ".paddlex-cache"
...
def _font_candidates() -> list[Path]:
    configured = os.environ.get("BCPICKUP_FONT_PATH")
...
def configure_offline_paddle_environment() -> Path:
    ...
    if font_path is None:
        raise ModelConfigurationError(
            "PaddleX用ローカルフォントがありません。BCPICKUP_FONT_PATHへ"
...
def verify_runtime_versions(manifest: dict) -> None:
    ...
    Pythonのminorまで見るのは、この環境での受け入れ評価が特定のPython上で測られて
    いるため。別minorは未評価の構成であり、CONTEXT.mdのRecognition Releaseでは
    新しい候補として再評価が要る（Project/backend/app/recognition_v2/model_runtime.py
    と契約を合わせている。2026-08-18、本番で3.13を掴んだ事故を機に追加）。"""
...
class PaddleModels:
    """固定済みPaddleモデルを1プロセス内で遅延生成して再利用する。"""
    ...
    def __init__(
        ...
        # Paddleの推論エンジンはスレッドセーフではない。並行呼び出し元（Project/backendの
        # V2は複数カードを同時にasyncio.to_threadへ投げる）で同じpredictorへ同時に
        # predict()が飛ぶとネイティブ側でSIGSEGVすることを本番で確認した(2026-08-18)。
        # pickup自体は現状シングルスレッド実行だが、契約を合わせておく。

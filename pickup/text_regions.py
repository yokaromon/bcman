...
class LocalTextPipeline:
    ...
    def process(self, image: np.ndarray, *, server: bool = True) -> dict:
        """Paddleの推論エンジン内部で稀に一過性の `RuntimeError: std::exception` が
        出ることを確認した（同一画像・同一プロセス内での直後の再実行は成功しており、
        再現性のある入力起因ではなさそうだった。2026-08-18、本番incident）。1回だけ
        再試行する。"""
        try:
            return self._process_once(image, server=server)
        except Exception:
            return self._process_once(image, server=server)

    def _process_once(self, image: np.ndarray, *, server: bool = True) -> dict:
        detected = []
